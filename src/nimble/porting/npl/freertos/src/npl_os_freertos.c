...
#if OS_MEM_ALLOC
    if (!os_memblock_from(&ble_freertos_ev_pool_ovr,ev->event)) {
        ev->event = os_memblock_get(&ble_freertos_ev_pool_ovr);
    }
#else
    if(!ev->event) {
...
#if OS_MEM_ALLOC
    os_memblock_put(&ble_freertos_ev_pool_ovr,ev->event);
#else
    free(ev->event);
#endif
    ev->event = NULL;
...
#if OS_MEM_ALLOC
    if (!os_memblock_from(&ble_freertos_evq_pool_ovr,evq->eventq)) {
        evq->eventq = os_memblock_get(&ble_freertos_evq_pool_ovr);
...
#if OS_MEM_ALLOC
    os_memblock_put(&ble_freertos_evq_pool_ovr,eventq);
#else
    free((void *)eventq);
#endif
    evq->eventq = NULL;
...
#if OS_MEM_ALLOC
    if (!os_memblock_from(&ble_freertos_mutex_pool_ovr,mu->mutex)) {
        mu->mutex = os_memblock_get(&ble_freertos_mutex_pool_ovr);
...
#if OS_MEM_ALLOC
    os_memblock_put(&ble_freertos_mutex_pool_ovr,mutex);
#else
    free((void *)mutex);
#endif
    mu->mutex = NULL;
...
#if OS_MEM_ALLOC
    if (!os_memblock_from(&ble_freertos_sem_pool_ovr,sem->sem)) {
        sem->sem = os_memblock_get(&ble_freertos_sem_pool_ovr);
...
#if OS_MEM_ALLOC
    os_memblock_put(&ble_freertos_sem_pool_ovr,semaphor);
#else
    free((void *)semaphor);
#endif
    sem->sem = NULL;
...
#if OS_MEM_ALLOC
    if (!os_memblock_from(&ble_freertos_co_pool_ovr, co->co)) {
        co->co = os_memblock_get(&ble_freertos_co_pool_ovr);
...
#if OS_MEM_ALLOC
    os_memblock_put(&ble_freertos_co_pool_ovr,callout);
#else
    free((void *)callout);
#endif
...
extern bool npl_freertos_os_started(void);
extern void *npl_freertos_get_current_task_id(void);
extern void npl_freertos_event_init(struct ble_npl_event *ev, ble_npl_event_fn *fn, void *arg);
extern void npl_freertos_event_deinit(struct ble_npl_event *ev);
extern void npl_freertos_event_reset(struct ble_npl_event *ev);
extern void npl_freertos_eventq_init(struct ble_npl_eventq *evq);
extern void npl_freertos_eventq_deinit(struct ble_npl_eventq *evq);
extern void npl_freertos_callout_mem_reset(struct ble_npl_callout *co);
extern struct ble_npl_event *npl_freertos_eventq_get(struct ble_npl_eventq *evq, ble_npl_time_t tmo);
extern void npl_freertos_eventq_put(struct ble_npl_eventq *evq, struct ble_npl_event *ev);
extern void npl_freertos_eventq_remove(struct ble_npl_eventq *evq, struct ble_npl_event *ev);
extern ble_npl_error_t npl_freertos_mutex_init(struct ble_npl_mutex *mu);
extern ble_npl_error_t npl_freertos_mutex_deinit(struct ble_npl_mutex *mu);
extern void npl_freertos_event_run(struct ble_npl_event *ev);
extern bool npl_freertos_eventq_is_empty(struct ble_npl_eventq *evq);
extern bool npl_freertos_event_is_queued(struct ble_npl_event *ev);
extern void *npl_freertos_event_get_arg(struct ble_npl_event *ev);
extern void npl_freertos_event_set_arg(struct ble_npl_event *ev, void *arg);
extern ble_npl_error_t npl_freertos_mutex_pend(struct ble_npl_mutex *mu, ble_npl_time_t timeout);
extern ble_npl_error_t npl_freertos_mutex_release(struct ble_npl_mutex *mu);
extern ble_npl_error_t npl_freertos_sem_init(struct ble_npl_sem *sem, uint16_t tokens);
extern ble_npl_error_t npl_freertos_sem_deinit(struct ble_npl_sem *sem);
extern ble_npl_error_t npl_freertos_sem_pend(struct ble_npl_sem *sem, ble_npl_time_t timeout);
extern ble_npl_error_t npl_freertos_sem_release(struct ble_npl_sem *sem);
extern void npl_freertos_callout_init(struct ble_npl_callout *co, struct ble_npl_eventq *evq,
                                      ble_npl_event_fn *ev_cb, void *ev_arg);
extern void npl_freertos_callout_deinit(struct ble_npl_callout *co);
extern uint16_t npl_freertos_sem_get_count(struct ble_npl_sem *sem);
extern ble_npl_error_t npl_freertos_callout_reset(struct ble_npl_callout *co, ble_npl_time_t ticks);
extern void npl_freertos_callout_stop(struct ble_npl_callout *co);
extern bool npl_freertos_callout_is_active(struct ble_npl_callout *co);
extern ble_npl_time_t npl_freertos_callout_get_ticks(struct ble_npl_callout *co);
extern ble_npl_time_t npl_freertos_callout_remaining_ticks(struct ble_npl_callout *co, ble_npl_time_t now);
extern void npl_freertos_callout_set_arg(struct ble_npl_callout *co, void *arg);
extern uint32_t npl_freertos_time_get(void);
extern ble_npl_error_t npl_freertos_time_ms_to_ticks(uint32_t ms, ble_npl_time_t *out_ticks);
extern ble_npl_error_t npl_freertos_time_ticks_to_ms(ble_npl_time_t ticks, uint32_t *out_ms);
extern ble_npl_time_t npl_freertos_time_ms_to_ticks32(uint32_t ms);
extern uint32_t npl_freertos_time_ticks_to_ms32(ble_npl_time_t ticks);
extern void npl_freertos_time_delay(ble_npl_time_t ticks);
extern uint32_t npl_freertos_hw_enter_critical(void);
extern uint8_t npl_freertos_hw_is_in_critical(void);
extern void npl_freertos_hw_exit_critical(uint32_t ctx);
extern uint32_t npl_freertos_get_time_forever(void);
...
void npl_freertos_mempool_deinit_ovr(void)
{
#if SOC_ESP_NIMBLE_CONTROLLER
    if(ble_freertos_ev_buf) {
        free(ble_freertos_ev_buf);
